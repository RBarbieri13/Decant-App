...
export function getSegments(): any[] {
  ...
  if (segments.length === 0) {
    ...
    segments = db.prepare('SELECT * FROM segments ORDER BY created_at').all() as any[];
  }
...
export function getOrganizations(): any[] {
  ...
  if (orgs.length === 0) {
    ...
    orgs = db.prepare('SELECT * FROM organizations ORDER BY created_at').all() as any[];
  }
...
function buildSubtree(parentId: string, parentField: string): any[] {
  const db = getDatabase();
